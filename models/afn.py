...
class ASPP_c(nn.Module):
    def __init__(self, dilation_list, channels, kernel_size, num_branches):
        ...
        for i in range(self.num_branches):
            self.aspp.append(nn.Conv3d(self.channels[8], self.channels[9], kernel_size=self.kernel_size,
                                       padding=self.padding_list[i], dilation=self.dilation_list[i]))
...
class ASPP_s(nn.Module):
    def __init__(self, dilation_list, channels, kernel_size, num_branches):
        ...
        for i in range(self.num_branches):
            self.conv_list.append(nn.Conv3d(self.channels[8], self.channels[9], kernel_size=self.kernel_size,
                                            padding=self.padding_list[i], dilation=self.dilation_list[i]))
    ...
    def __init__(self, blocks, padding_list, dilation_list, channels, kernel_size, num_branches):
        ...
        for i in range(len(blocks)):
            ...
            if block == BasicBlock:
                ...
                self.layers.append(
                    block(self.channels[index], self.channels[index + 1], self.channels[index + 2], self.padding_list,
