...
from models.se_resnext import se_resnext50
...
class UNet16(nn.Module):
    def __init__(self, num_classes=1, num_filters=32, pretrained=True, requires_grad=True):
        """
        :param num_classes:
        :param num_filters:
        :param pretrained:
...
class AlbuNet(nn.Module):
    ...
    def __init__(self, num_classes=1, num_filters=32, pretrained=True, is_deconv=False, requires_grad=True):
        """
        :param num_classes:
        :param num_filters:
        :param pretrained:
...
class SeRes50NextHyper(nn.Module):
    ...
    def __init__(self, num_classes=1, num_channels=3, num_filters=32, pretrained=True, is_deconv=False,
        ...
        self.encoder = se_resnext50(requires_grad=requires_grad)
        ...
        self.dec5 = DecoderBlockV2(2304, num_filters * 8 * 2, num_filters * 8, is_deconv)
        self.dec4 = DecoderBlockV2(1280, num_filters * 8 * 2, num_filters * 8, is_deconv)
        self.dec3 = DecoderBlockV2(768, num_filters * 4 * 2, num_filters * 2, is_deconv)
        self.dec2 = DecoderBlockV2(320, num_filters * 2 * 2, num_filters * 2 * 2, is_deconv)
        self.dec1 = DecoderBlockV2(num_filters * 2 * 2, num_filters * 2 * 2, num_filters, is_deconv)
        # self.dec0 = ConvRelu(256, num_filters)
        self.final = nn.Conv2d(num_filters, num_classes, kernel_size=1)

    def forward(self, x):
        prt = False
        conv1 = self.conv1(x)
        if prt: print('\nconv1 ' + str(conv1.shape))
        conv2 = self.conv2(conv1)
        if prt: print('conv2 ' + str(conv2.shape))
        conv3 = self.conv3(conv2)
        if prt: print('conv3 ' + str(conv3.shape))
        conv4 = self.conv4(conv3)
        if prt: print('conv4 ' + str(conv4.shape))
        conv5 = self.conv5(conv4)
        if prt: print('conv4 ' + str(conv4.shape))

        f = self.center(conv5)
        if prt: print('center ' + str(f.shape))

        dec5 = self.dec5(torch.cat([f, conv5], 1))
        if prt: print('dec5 ' + str(dec5.shape))
        dec4 = self.dec4(torch.cat([dec5, conv4], 1))
        if prt: print('dec4 ' + str(dec4.shape))
        dec3 = self.dec3(torch.cat([dec4, conv3], 1))
        if prt: print('dec3 ' + str(dec3.shape))
        dec2 = self.dec2(torch.cat([dec3, conv2], 1))
        if prt: print('dec2 ' + str(dec2.shape))
        dec1 = self.dec1(dec2)
        if prt: print('dec1 ' + str(dec1.shape))
        if self.hp:
            f = torch.cat([dec1,
        ...
        # dec0 = self.dec0(f)
        # if prt: print('dec0 ' + str(dec0.shape))

        # if self.num_classes > 1:
        # x_out = F.log_softmax(self.final(dec0), dim=1)
        # else:
        x_out = self.final(dec1)
        if prt: print('x_out ' + str(x_out.shape))
        if prt: exit()
        return x_out
